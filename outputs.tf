...
output "vm_resource_group_name" {
  ...
  value = azurerm_resource_group.mytfrg.name
...
output "vm_resource_group_location" {
  ...
  value = azurerm_resource_group.mytfrg.location
}
...
output "vm_network_interface_ids" {
  ...
  value = [azurerm_linux_virtual_machine.myterraformvm.network_interface_ids]
}
